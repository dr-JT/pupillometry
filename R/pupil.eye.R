...
pupil.eye <- function(x, eye.recorded = "", eye.use = "", gazedata = FALSE){
  ...
  if (eye.recorded == "both"){
    ...
    if (eye.use=="left"){
      ...
      x <- dplyr::select(x, -R_Pupil_Diameter.mm, -R_Event, -R_Gaze_Position.x, -R_Gaze_Position.y)
      ...
      x <- dplyr::select(x, -L_Pupil_Diameter.mm, -L_Event, -L_Gaze_Position.x, -L_Gaze_Position.y)
    }
