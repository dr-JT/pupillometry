...
eye.method <- function(x, eye.recorded = "", method = "average"){
    ...
    if (method=="average"){
      ...
      if (eyes.corr>=.90){
        data <- dplyr::mutate(data, Pupil_Diameter.mm = rowMeans(dplyr::select(data, L_Pupil_Diameter.mm, R_Pupil_Diameter.mm), na.rm = TRUE),
                              ...
                              Missing.Consecutive = rowMeans(dplyr::select(data, L_Missing.Consecutive, R_Missing.Consecutive), na.rm = TRUE),
                              Eye = "Average", Eyes.r = eyes.corr)
        data <- dplyr::select(data, -L_Pupil_Diameter.mm, -R_Pupil_Diameter.mm,
                              -L_Missing.Total, -R_Missing.Total, -L_Missing.Consecutive, -R_Missing.Consecutive)
      } else {
        if (left.missing<=right.missing){
          data <- dplyr::mutate(data, Eye = "Left", Eyes.r = eyes.corr)
          data <- dplyr::select(data, -R_Pupil_Diameter.mm, -R_Missing.Total, -R_Missing.Consecutive)
          data <- dplyr::rename(data, Pupil_Diameter.mm = L_Pupil_Diameter.mm,
                                Missing.Total = L_Missing.Total, Missing.Consecutive = L_Missing.Consecutive)
        } else {
          data <- dplyr::mutate(data, Eye = "Right", Eyes.r = eyes.corr)
          data <- dplyr::select(data, -L_Pupil_Diameter.mm, -L_Missing.Total, -L_Missing.Consecutive)
          data <- dplyr::rename(data, Pupil_Diameter.mm = R_Pupil_Diameter.mm,
                                Missing.Total = R_Missing.Total, Missing.Consecutive = R_Missing.Consecutive)
        }
      }
