  ...
  if (is.null(aggregate)) {
    ...
    for (group in unique(x[[aggregate]])) {
      ...
      plot <- ggplot2::ggplot(data_group,
                              ggplot2::aes(Time, pupil_val_before, group = aggregate)) +
        ggplot2::stat_summary(ggplot2::aes(x = Time, y = pupil_val_before, group = aggregate),
                              fun = mean, na.rm = TRUE,
                              geom = "line", alpha = .35) +
        ggplot2::stat_summary(ggplot2::aes(x = Time, y = pupil_val_after, group = aggregate),
                              fun = mean, na.rm = TRUE,
                              geom = "line") +
