...
To use `plot_comparison()` you first need to create the preprocessed data files with different parameters (from a single raw data file) with `preprocess()`. You may need to rename the file after running each `preprocess()` so that it doesn't get overwritten. 
