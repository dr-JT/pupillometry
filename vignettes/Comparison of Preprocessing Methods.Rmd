...
With `plot_comparison()` you can visually compare how different preprocessing parameters change the shape of the pupil data.

To use `plot_comparison()` you first need to create the preprocessed data files with different parameters (from a single raw data file) with `pupil_preprocess()`. You may need to rename the file after running each `pupil_preprocess()` so that it doesn't get overwritten. 
...
```r
plot_comparison(import = "data/test", 
...
Here are some examples from `plot_comparison()` that demonstrate important differences when choosing whether to interpolate or smooth the data first.
